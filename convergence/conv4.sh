#!/bin/bash
#Lrlrlr 281019 conv4.sh
#to calculate newRMSD and tmscore and merge with previous tmalign results
#can run on MAGNUS 
#try clustering using maxcluster64bit.

#make sure already have native.pdb in each *_cv folder
...
do
	cd "$d"	
	GLOBIGNORE=*.pdb.tar.bz2:tmalign.*:native.*:*.score.fsc
	rm -v *
	unset GLOBIGNORE
	...
	sed 's/(//' temp.max > temp.max2 #remove "("
	sed '$d' temp.max2 > maxi #remove the last line which is the native
	sed -i "s/ //g" maxi	#remove whitespace in file
	cat tmalign.log | grep -o -P '(?<=./).*(?=.pdb)' > id.log		#Start creating data for RMSD.csv, capture only id value of decoys
	...
	paste id.log rmsd.log tmscore.log | column -s $'\t' -t > temp1.csv
	grep -v "native" temp1.csv > idalignscore #delete line containing native in temp1.csv
	paste idalignscore maxi | column -s $'\t' -t > tmalignRMSDscore.csv	#create RMSD.csv file #match rmsd value with models id
	cat tmalignRMSDscore.csv | awk '{print $1, $4}' > RMSD.csv
	rm id.log rmsd.log 
	cd ..	#go back to parent directory, and run other folders
done 

#evaluation plots using the newRMSD, adapt from
#Lrlrlr 300519 conv3M.sh 

#plots using tmscore values
#!/bin/bash
#Lrlrlr 300519 conv3M.sh
...
#need to use delimiters to sort pdbID in incremental order for correct merging between RMSD.csv and score.fsc
#hv to remove header in score.fsc and native line in RMSD.csv
