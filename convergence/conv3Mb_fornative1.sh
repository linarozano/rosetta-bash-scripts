...
do
	...
	sed -i '/native/d' tmalign1.csv	#process tmalign.csv (ID, rmsd) -no header but need to remove native line (will change source file)
	cat tmalign1.csv | awk '{print $1}' > temp_IDt.txt 
	cut -d '_' -f 2 temp_IDt.txt > temp_IDt1.txt
	paste tmalign1.csv temp_IDt1.txt | column -s $'\t' -t > aligntm.txt	#merge both files
