#!/bin/bash
#Lrlrlr 160619 updated 200619 cluster_optimize.sh
...
cluster.mpi.linuxgccrelease -in:file:s *.pdb -in:file:fullatom -run:shuffle -cluster:radius 3.0 -cluster:limit_clusters -1 -cluster:sort_groups_by_energy -out:prefix cluster3 > cluster3.log
mkdir Cluster3
mv cluster3* Cluster3
rm cluster3*
echo "Cluster3 DONE!"

cluster.mpi.linuxgccrelease -in:file:s *.pdb -in:file:fullatom -run:shuffle -cluster:radius 4.0 -cluster:limit_clusters -1 -cluster:sort_groups_by_energy -out:prefix cluster4 > cluster4.log
mkdir Cluster4
mv cluster4* Cluster4
rm cluster4*
echo "Cluster4 DONE!"

cluster.mpi.linuxgccrelease -in:file:s *.pdb -in:file:fullatom -run:shuffle -cluster:radius 5.0 -cluster:limit_clusters -1 -cluster:sort_groups_by_energy -out:prefix cluster5 > cluster5.log
mkdir Cluster5
mv cluster5* Cluster5
rm cluster5*
echo "Cluster5 DONE!"

cluster.mpi.linuxgccrelease -in:file:s *.pdb -in:file:fullatom -run:shuffle -cluster:radius 6.0 -cluster:limit_clusters -1 -cluster:sort_groups_by_energy -out:prefix cluster6 > cluster6.log
mkdir Cluster6
