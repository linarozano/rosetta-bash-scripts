#!/bin/bash
#Lrlrlr 160619 updated 280719 cluster_filterrun.sh
#Custom for runs that didnt finish clustering due to too many models below0.
#Set limit for top 10,000 taken from below0.scoretmalign. named cutoff10k
#set for 20k-50k
...
cat below0.scoretmalign | sort -nk 3 | head -10000 > cutoffenergy10k
cat cutoffenergy10k | sed '/^$/d' | awk '{print $1".pdb"}' > PDBlistcutoff10k

#extract pdb of only below 0
tar xjf *pdb.tar.bz2 -T PDBlistcutoff10k
...
do
	...
	paste cutoffenergy10k temp_merge | column -s $'\t' -t > cluster_table
